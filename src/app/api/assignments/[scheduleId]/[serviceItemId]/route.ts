...
export async function PUT(
  ...
  try {
    ...
    if (schedule.is_locked === 1) return jsonError(schedule.lock_message || "此日期已鎖定，無法修改登記", 403);
    ...
    ).bind(scheduleId, serviceItemId).first() as { member_id: number | null; custom_member_name: string | null } | null;
